...
public class Sensor {
	...
	public void Cast()
	{
		...
		//Temporarily change all colliders in the the ignoreList to triggers;
		//The raycast will ignore all triggers by default; As a result, all collidera in the list are ignored;
		foreach(Collider _col in ignoreList)
			_col.isTrigger = true;
		...
		//Change back all colliders in ignoreList;
		foreach(Collider _col in ignoreList)
	...
	public Sensor (Transform _transform, Collider _collider)
	{
		...
		if(!ignoreList.Contains(_collider))
			ignoreList.Add(_collider);
	}
